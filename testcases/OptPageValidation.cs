...
{
    [TestClass]
    public class OptPageValidation : BaseUITest
    {
        ...
        public async Task SkipOtpForTrustedDeviceAsync()
        {
            ...
            if (afterLoginPage is OtpPage otpPage)
            {
                ...
                if (isTrusted)
                {
                    Assert.IsTrue(redirectedPage is HomePage, "Should skip OTP and land on HomePage for trusted device.");
        ...
        public async Task LoginToOtpPageSuccessfully()
        {
            ...
            if (afterLoginPage is OtpPage)
                return; // Parallel to NUnit Assert.Pass
            else if (afterLoginPage is HomePage)
        ...
        [TestMethod]
        [Description("Verify that Otp enabled user is sended otp code")]
        public async Task OtpSmsSendedSuccessfully()
        {
            ...
            var (otpGeneratedTime, otp, smsSent) = dbHelper.GetLatestOtpCode(OtpUserLogin, ConnectionString);
            TestContext.WriteLine(smsSent.ToString());
            TestContext.WriteLine(otpGeneratedTime?.ToString() ?? "null");
        ...
        [TestMethod]
        [Description("Verify if opt code still active within 5 minutes")]
        public async Task ValidateOtpAlreadySendAsync()
        {
            ...
            if (redirectedPage is OtpPage otpResendPage)
            {
                ...
                Assert.IsTrue(hasValidation, "Expected OTP already sent validation error not found.");
            }
            else
            {
        ...
        [TestMethod]
        [Description("Verify if user can login with active otp code")]
        public async Task LoginWithValidOtpAsync()
        {
            ...
            {
                // Successfully logged in with correct OTP
                return;
            }
            else
            {
